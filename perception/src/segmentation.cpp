...
namespace perception {
  ...
  void Segmenter::Callback(const sensor_msgs::PointCloud2& msg) {
    ...
    for (size_t i = 0; i < objects.size(); ++i) {
       ROS_INFO("Publishing object %ld in segmenter callback", i);
      ...
      ROS_INFO("Object pose x: %lf", object.pose.position.x);
      ROS_INFO("Object pose y: %lf", object.pose.position.y);
      ROS_INFO("Object pose z: %lf", object.pose.position.z);
      ...
      std::stringstream ss;
      ss << name << " (" << confidence << ")";
      ROS_INFO("Confidence %lf", confidence);
